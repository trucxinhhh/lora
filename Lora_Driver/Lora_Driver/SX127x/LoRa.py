...
def getter(register_address):
	...
	def decorator(func):
		def wrapper(self):
			GPIO.output(BOARD.NSS, 0)
			payloads = func(self, self.spi.xfer([register_address, 0])[1])
...
def setter(register_address):
	...
	def decorator(func):
		def wrapper(self, val):
			GPIO.output(BOARD.NSS, 0)
			payloads = self.spi.xfer([register_address | 0x80, func(self, val)])[1]
			GPIO.output(BOARD.NSS, 1)
			return payloads
		return wrapper
	return decorator
...
class LoRa(object):
	...
	def set_mode(self, mode):
		...
		self.mode = mode
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.OP_MODE | 0x80, mode])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def write_payload(self, payload):
		...
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO | 0x80] + payload)[1:]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def set_freq(self, f):
		...
		#print(msb, mid, lsb)
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FR_MSB | 0x80, msb, mid, lsb])
		GPIO.output(BOARD.NSS, 1)
		return 
	...
	def set_pa_config(self, pa_select=None, max_power=None, output_power=None):
		...
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.PA_CONFIG | 0x80, val])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def set_ocp_trim(self, I_mA):
		...
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.OCP | 0x80, v])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def get_fifo_addr_ptr(self):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_ADDR_PTR, 0])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def set_fifo_addr_ptr(self, ptr):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_ADDR_PTR | 0x80, ptr])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def get_fifo_tx_base_addr(self):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_TX_BASE_ADDR, 0])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def set_fifo_tx_base_addr(self, ptr):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_TX_BASE_ADDR | 0x80, ptr])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def get_fifo_rx_base_addr(self):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_RX_BASE_ADDR, 0])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def set_fifo_rx_base_addr(self, ptr):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_RX_BASE_ADDR | 0x80, ptr])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def get_fifo_rx_current_addr(self):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_RX_CURR_ADDR, 0])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def get_fifo_rx_byte_addr(self):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.FIFO_RX_BYTE_ADDR, 0])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def set_irq_flags_mask(self,
		...
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.IRQ_FLAGS_MASK | 0x80, v])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def set_irq_flags(self,
		...
		for i, s in enumerate(['cad_detected', 'fhss_change_ch', 'cad_done', 'tx_done', 'valid_header',
			...
			if this_bit is not None:
				v = set_bit(v, i, this_bit)
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.IRQ_FLAGS | 0x80, v])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def get_rx_nb_bytes(self):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.RX_NB_BYTES, 0])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def set_modem_config_1(self, bw=None, coding_rate=None, implicit_header_mode=None):
		...
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.MODEM_CONFIG_1 | 0x80, val])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def set_modem_config_2(self, spreading_factor=None, tx_cont_mode=None, rx_crc=None):
		...
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.MODEM_CONFIG_2 | 0x80, val])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def set_modem_config_3(self, low_data_rate_optim=None, agc_auto_on=None):
		...
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([REG.LORA.MODEM_CONFIG_3 | 0x80, val])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
	...
	def get_register(self, register_address):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([register_address & 0x7F, 0])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads

	def set_register(self, register_address, val):
		GPIO.output(BOARD.NSS, 0)
		payloads = self.spi.xfer([register_address | 0x80, val])[1]
		GPIO.output(BOARD.NSS, 1)
		return payloads
